...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        setIsLoading(true);
        const [usersRes, userRes, tasksRes] = await Promise.all([
  ...
  const assignedTasks = tasks.filter((task) => {
    const assignedToId = task.assignedTo?._id || task.assignedTo; // Handle string or object
    console.log('Filtering assigned task:', { taskId: task._id, assignedToId, userId: user?._id }); // Debug log
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {showTaskForm && (
          <div className="mb-8">
            <TaskForm
              users={users}
              onTaskCreated={() => {
                console.log('onTaskCreated triggered');
                setShowTaskForm(false);
                const token = localStorage.getItem('token');
                axios
                  .get(`${process.env.NEXT_PUBLIC_API_URL}/api/tasks`, {
                  ...
                  .then((res) => {
                    console.log('Tasks refreshed:', res.data);
                    setTasks(res.data);
                  })
                  .catch((error) => console.error('Error fetching tasks:', error));
              }}
        ...
        <div className="flex flex-col md:flex-row gap-4 mb-6 items-center justify-between">
          ...
          <div className="flex items-center space-x-2">
            ...
            <button
              onClick={() => {
                console.log('Toggling showTaskForm:', !showTaskForm);
                setShowTaskForm(!showTaskForm);
              }}
              className="bg-indigo-600 text-white px-4 py-2 rounded"
            >
                ...
                <div className={view === 'grid' ? 'grid grid-cols-1 md:grid-cols-3 gap-4' : 'space-y-4'}>
                  {assignedTasks.map((task) => (
                    <TaskCard
                      ...
                      users={users}
                      onTaskUpdated={() => {
                        console.log('onTaskUpdated triggered');
                        const token = localStorage.getItem('token');
                        axios
                          .get(`${process.env.NEXT_PUBLIC_API_URL}/api/tasks`, {
                            headers: { Authorization: `Bearer ${token}` },
